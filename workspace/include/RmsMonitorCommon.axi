//*********************************************************************
//
//             AMX Resource Management Suite  (4.5.3)
//
//*********************************************************************
/*
 *  Legal Notice :
...
(*        by any user implementation code.                 *)
(***********************************************************)
DEFINE_FUNCTION RmsEventAssetRegistered(CHAR registeredAssetClientKey[], LONG assetId, CHAR newAssetRegistration)
{
  ...
  // relay the notification via callback method invocation
  RmsEventRelayAssetRegistered(registeredAssetClientKey, newAssetRegistration);
  ...
  IF(registeredAssetClientKey == assetClientKey)
  {
...
{
  SEND_STRING 0, "'[',MONITOR_DEBUG_NAME,'-',RmsDevToString(dvMonitoredDevice),'] ',data";
}
...
DEFINE_FUNCTION PrintVersion()
{
  ...
  SEND_STRING 0, "name,' : ',MONITOR_VERSION,' (',RmsDevToString(dvMonitoredDevice),')'";
}
...
DEFINE_FUNCTION RegisterAssetMetadataWrapper()
{
  ...
  // register asset metadata properties
  // with RMS now
  IF((DEVICE_ID(dvMonitoredDevice) && assetRegistered) &&
