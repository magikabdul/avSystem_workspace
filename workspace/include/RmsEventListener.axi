...
// #DEFINE INCLUDE_RMS_EVENT_RFID_INITIALIZE_CALLBACK
DEFINE_FUNCTION RmsEventRfidInitialize()
{
}
  ...
  COMMAND:
  {
    ...
    SELECT
    {
      ...
      ACTIVE (rmsHeader == RMS_EVENT_ASSET_REGISTERED):
      {
        ...
        RmsEventAssetRegistered(rmsParam1,ATOI(rmsParam2),RmsBooleanValue(rmsParam3));
      }
      #END_IF
      ...
      // (RMS RFID INITIALIZE EVENT NOTIFICATION)
      //
      // upon receiving the RFID initialization notification from
      // the RMS client, relay this event to any event subscribers
      //
      #IF_DEFINED INCLUDE_RMS_EVENT_RFID_INITIALIZE_CALLBACK
      ACTIVE (rmsHeader == RMS_EVENT_RFID_INITIALIZE):
      {
        // invoke the callback method
        RmsEventRfidInitialize();
      }
      #END_IF
